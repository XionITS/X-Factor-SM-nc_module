...
def plug_in_service(data, time):
    ...
    for d in proc_data:
        ...
        if time == 'minutely':
            ...
            computer_id2 = d[0][0]['text']
            # print(computer_id2)
            common = Xfactor_Common_log.objects.all()
            for item in common:
                # print(item.computer_id)
                common_id = item.computer_id
            ...
            print(type(common_id))
            xfactor_service_log = Xfactor_Service_log.objects.update_or_create(computer=common_id, **defaults)
            print("tlqkf")
    ...
    for d in proc_data:
        ...
        try:
            if time == 'minutely':
                ...
                common_id = Xfactor_Common_log.objects.all().filter(user_date__date=now.date())
                for record in common_id:
                    common_id = record.computer_id
        ...
        if time == 'minutely':
            ...
            #xfactor_purchase, created = Xfactor_Purchase.objects.update_or_create(computer=common_id,  **defaults)
